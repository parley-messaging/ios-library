...
class ParleyCustomerAuthorization {
    ...
    */
    @available(*, deprecated, message: "Always generate the user authorization for Parley on a safe and secure location (never on a client device). Use this method for test purposes only.")
    static func generate(identification: String, secret: String, sharedSecret: String) -> String {
        // 1. Customer authentication key
        let customerAuthenticationKey = self.hmac(identification, withKey: secret)
...
//        debugPrint("Result: \(customerAuthenticationKey)")
        ...
        let verifyHash = self.hmac(verifyHashDataString, withKey: sharedSecret)
...
//        debugPrint("Result: \(verifyHash)")
        ...
        let authenticationEncodedDataArray = [identification, customerAuthenticationKey, validUntillTimestampString, verifyHash];
...
//        debugPrint("Result: \(userAuthentication)")
    ...
    private static func hmac(_ plainText: String, withKey key: String) -> String {
        ...
        CCHmac(CCHmacAlgorithm(kCCHmacAlgSHA512), cKey, key.lengthOfBytes(using: .utf8), cData, plainText.lengthOfBytes(using: .utf8), cHMAC)
