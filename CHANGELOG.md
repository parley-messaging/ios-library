...
- [Styling] Added `imageTimeBackgroundColor` to `ParleyMessageViewAppearance`, to set the color.
- [Styling] Added `imageTimeSpacing` to `ParleyMessageViewAppearance`, to adjust the spacing between the icon and the time.
- [Styling] Added `imageTimeHorizontalPadding` to `ParleyMessageViewAppearance` to modify the horizontal padding around the time label, ensuring proper spacing within the background.
